...
import { BookOpen, Check, FileText, Globe, Link2, Plus, Sparkles } from "lucide-react";
...
export function MagicVideoSourceScreen({ embedded = false }: { embedded?: boolean }) {
  ...
  const content = (
    <main className="mx-auto w-full max-w-[1280px] px-6 py-7 sm:px-8">
            ...
            {DOSSIERS.map((d) => {
              ...
              return (
                <div
                  key={d.id}
                  onClick={() => {
                    setSelectedDossier(d.id);
                    setSourcePayload({ dossierId: d.id });
                  }}
          ...
          <div className="mt-4 flex h-12 items-center gap-3 rounded-[14px] border border-[var(--hair-2)] bg-[#fcfdfc] px-3.5 focus-within:border-[var(--brand)] focus-within:bg-white focus-within:ring-2 focus-within:ring-[var(--brand-soft)]">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setSourcePayload({ url: e.target.value });
              }}
        ...
        <div className="rise-in mt-6 rounded-[22px] border border-[var(--hair-2)] bg-white p-6 shadow-sm">
          ...
          <textarea
            ...
            onChange={(e) => {
              ...
              setSourcePayload({ text: e.target.value });
            }}
            rows={5}
      ...
      {/* When not embedded, show standalone continue button */}
      {!embedded && (
